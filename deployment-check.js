...
const researchConfig = fs.readFileSync(path.join(root, 'research-config.js'), 'utf8');
check('Configured Apps Script URL', /https:\/\/script\.google\.com\/macros\/s\/[A-Za-z0-9_-]+\/exec/.test(researchConfig), 'Production /exec endpoint inserted');
check('Apps Script sign-in diagnostic', /ENDPOINT_LOGIN_REQUIRED/.test(html) && /Deploy semula dengan akses Anyone/.test(html), 'Clear guidance when deployment redirects to Google Sign-In');
...
check('Single Tab 08 instruction flow', (html.match(/<div class="challenge-purpose">/g) || []).length === 1, 'One Ask → Choose → Copy block');
